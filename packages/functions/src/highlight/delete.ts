...
export async function main(event: APIGatewayProxyEvent) {
  try {
    ...
    await dynamoDb.send(new DeleteCommand({ TableName: TABLE_NAME, Key: { highlightId } }));


    return { statusCode: 200, body: JSON.stringify({ message: "Highlight deleted successfully" }) };
  } catch (error) {
